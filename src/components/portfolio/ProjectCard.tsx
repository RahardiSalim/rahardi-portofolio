...
export function ProjectCard({ project, index }: ProjectCardProps) {
  ...
  const relatedCompId = project.relatedCompetitions?.[0];
  // Strip leading "number." prefix from competition id to get slug
  const competitionSlug = relatedCompId ? relatedCompId.replace(/^\d+\./, '') : null;
  ...
  return (
    <div className="group h-full flex flex-col">
      ...
      {/* Competition link outside the main card link to avoid nested <a> */}
      {competitionSlug && (
        <Link
