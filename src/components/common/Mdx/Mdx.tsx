    ...
    <div className={`prose dark:prose-invert max-w-none ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ ...props }) => (
            <a
              className="text-blue-600 dark:text-blue-400 hover:underline underline-offset-4 decoration-1"
              target="_blank"
          ...
          code: ({ ...props }) => (
            <code
              className="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded font-mono text-sm text-pink-600 dark:text-pink-400"
              {...props}
            />
          ),
          pre: ({ ...props }) => (
            <pre
              className="bg-gray-900 text-gray-100 p-6 rounded-none border border-black dark:border-gray-700 overflow-x-auto mb-6 font-mono text-sm"
              {...props}
            />
          ),
          blockquote: ({ ...props }) => (
            <blockquote
              className="border-l-4 border-blue-500 pl-6 italic text-gray-600 dark:text-gray-400 mb-6"
              {...props}
            />
          ),
        }}
